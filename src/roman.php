...
$sql = "SELECT * FROM `livres` WHERE `genre` LIKE 'Roman' ORDER BY `genre` ASC";
$query = $db->prepare($sql);
$query->execute();
$news = $query->fetchAll(PDO::FETCH_ASSOC);

// Récupérer les livres roman par sous-genre
$genres = ['Aventure', 'Fantastique', 'Policier', 'Romance', 'Science-fiction'];
$livres_genre = [];

foreach ($genres as $genre) {
    $sql = "SELECT * FROM `livres` WHERE `genre` LIKE 'Roman' AND `sous_genre` = ?";
    $query = $db->prepare($sql);
    $query->execute([$genre]);
...
<body>
    ...
    <section>
        <div class="categories">
            ...
            <div class="wrap">
                <h2><?= $genre ?></h2>
                <?php if (!empty($livres_genre[$genre])): ?>
                <div class="pad_carte">
